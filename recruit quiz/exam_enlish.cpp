...
{
	const struct {
		const char* phrase; //定形文の意味
		const char* meaning; //定形文の意味
		...
		2,{3,5}},
		{"be looking forward to [A]","[A]を楽しみ7にする",
	...
	case 1: //定型文の意味を答える
		for (int i = 0; i < quizCount; i++) {
			...
			s += std::string("	2:") + data[answers[2]].meaning + "\n";
			s += std::string("	3:") + data[answers[3]].meaning;
	...
	case 2://意味の合う定型文を答える
		for (int i = 0; i < quizCount; i++) {
			...
			s += std::string("	2:") + data[answers[2]].phrase + "\n";
			s += std::string("	3:") + data[answers[3]].phrase;
